...
class ActionSequenceTest : FunSpec(
    {
        context("rotation") {
            ...
            test("simple rotate") {
                ...
                val actionDef = testSequence.build()[0]
            ...
            test("rotate random") {
                ...
                val testSequence = sequence {
                    action {
                        rotator1 rotate angle1
                        rotator2 rotate angle2
                        rotator3 rotate angle3
                    }
                }
        ...
        context("multi-run") {
            ...
            test("re-runnable") {
                ...
                for (i in 1..2) {
                    val firstActionDef = listOfActions[0]
                    val secondActionDef = listOfActions.last()

                    val firstAction = firstActionDef.build().first
                    val secondAction = secondActionDef.build().first

                    // N.B. moving to an absolute angle takes 1 less step than the angle
                    val numberOfMoves = abs(randomAngle) * 2 - 2
            ...
            test("repeated with external influences") {
                ...
                val actionDef = testSequence.build()[0]
                val resetDef = testSequence.build()[1]

                for (i in 0 until 5) {
                    angleIndex = i
                    val randomAction = actionDef.build().first
                    val resetAction = resetDef.build().first
